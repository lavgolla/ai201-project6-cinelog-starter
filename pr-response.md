...
## Comment 6 — Rebase
**What conflicted:** `models.py` conflicted due to upstream main commit `07ca580` ("refactor: migrate film IDs from integer to UUID"). That commit changed `Film.id` from `db.Column(db.Integer, primary_key=True, autoincrement=True)` to `db.Column(db.String(36), default=generate_uuid)` and `CollectionEntry.film_id` from `db.Integer` to `db.String(36)`. This branch had added `WatchlistEntry` with `film_id = db.Column(db.Integer, ...)`, which conflicted with the UUID schema. After rebasing, `WatchlistEntry` was also missing entirely from `models.py`.

**How I resolved it:** Adopted the UUID schema from main — the correct resolution since main is the source of truth for the data model. Added `WatchlistEntry` to `models.py` with `film_id = db.Column(db.String(36), db.ForeignKey("film.id"), ...)` to match `Film.id`'s UUID type. Updated the `watchlist_service.py` docstring to reflect `film_id` is now a UUID string, and updated the test's fake `film_id` from integer `999999` to UUID string `"00000000-0000-0000-0000-000000000000"` to match the new schema.

**How I verified no conflict remains:** Ran `git status` — no conflict markers. Ran `pytest tests/test_watchlist.py -v` and `pytest tests/test_collection.py -v` — all 5 tests pass. Ran `grep -r "<<<<<<" . --include="*.py"` — no output confirming no leftover markers.
